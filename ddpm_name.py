...
def test_hyber_seq(self, seq, is_train=False):
        ddpm = False #True
        ...
        for i, (inputs_xyz, input_2d, targets_3d) in enumerate(dataloader):
            ...
            mpjpe_in_out50.append(mpjpe(output_xyz, inputs_xyz).item()*1000)

            if i % 1000 == 0:
                current_time = datetime.datetime.now()
                print(str(i) + " / " + str(len(dataloader)) + " " + "\t" + str(current_time))      
                    
                out_table = pd.DataFrame({'mpjpe_in_gt':mpjpe_in_gt, 'mpjpe_in_out50':mpjpe_in_out50, 'mpjpe_out_gt':mpjpe_list})
                out_table.to_csv(config.file_csv_name)
        
        out_table = pd.DataFrame({'mpjpe_in_gt':mpjpe_in_gt, 'mpjpe_in_out50':mpjpe_in_out50, 'mpjpe_out_gt':mpjpe_list})
        out_table.to_csv(config.file_csv_name)
        return output_xyz_list, mpjpe_list, epoch_loss_3d_pos_procrustes.avg
...
def parse_args_and_config(config):
    ...
    parser.add_argument("--mode", type=str, default='ddpm', help="ddpm/ddim")
...
def configs(runner, config):
    if config.mode == 'ddpm':
        config.file_csv_name = 'results/' +  "DDPM_" +'seq.csv'     
        config.seq = range(50)

    elif config.mode == 'ddim':
        config.file_csv_name = 'results/' +  "DDIM_" +'seq.csv'   
        config.seq = [0, 6]
...
def main():
    ...
    data_out = test_hyber_seq(runner, config.seq)
